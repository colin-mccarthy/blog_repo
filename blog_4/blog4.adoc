...
bad code to your repo. It will also save your CI from running and then blocking your commit. If you set up Travis CI to use yaml-lint you could also set up a pre-commit
hook that also uses yaml-lint and then you should never submit code that gets rejected by your Travis CI. This is another one of those developer tools that I really
